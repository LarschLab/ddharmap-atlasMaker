...
function activeLabels() {
  const byId = new Map(state.metadata.layers.map(layer => [layer.id, layer]));
...
function renderLayerList() {
  ...
  list.innerHTML = layers.map(layer => `
    <div class="layer-row ${state.active.has(layer.id) ? "active" : ""}" data-id="${layer.id}" title="${escapeHtml(layer.registered_subject)}">
...
class VolumeStore:
    ...
    def get(self, layer_id: str) -> np.ndarray:
        with self._lock:
            volume = self._cache.get(layer_id)
...
def composite_rgb(
    volumes: list[tuple[np.ndarray, str]],
    plane: str,
    index: int | str,
    brightness: float,
    contrast: float,
    ...
    first_volume = reference_volume if reference_volume is not None else volumes[0][0]
    ...
    for volume, color in volumes:
        norm = normalize_plane(plane_slice(volume, plane, index), brightness, contrast)
        color_rgb = np.array(hex_to_rgb(color), dtype=np.float32) / 255.0
...
class ViewerHandler(BaseHTTPRequestHandler):
    ...
    def handle_composite(self, query: dict[str, list[str]]) -> None:
        ...
        volumes = [
            (self.store.get(layer_id), self.layer_by_id[layer_id].color)
